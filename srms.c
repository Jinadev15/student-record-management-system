...
#define DEVELOPER_NAME   "Your Name"
...
void load_sample_data(Student **list, int *count, int *capacity)
{
    ...
    *count = 5;
    ...
    /* ── Live Union Demonstration (Unit VI) ── */
    printf(COLOR_CYAN
        "\n  [Union Demo] Demonstrating shared memory in union:\n"
        COLOR_RESET);

    /* Currently result.grade holds 'F' for Vikram */
    printf("  Before: result.grade = '%c'  "
           "(grade stored in union)\n",
           (*list + 4)->result.grade);

    /* Now write to pass_status — this OVERWRITES grade */
    (*list + 4)->result.pass_status = 0;
    printf("  After setting pass_status = 0:\n");
    printf("  result.pass_status = %d  (valid)\n",
           (*list + 4)->result.pass_status);
    printf("  result.grade = '%c'  "
           "(now INVALID — overwritten by pass_status)\n",
           (*list + 4)->result.grade);
    printf("  [Union Rule] Only the LAST written member "
           "is valid.\n\n");

    /* Restore grade for correct display */
    (*list + 4)->result.grade =
...
input_error:                /* goto label — used for input validation retry (Unit II) */
    ...
    /* ── Age Input with do-while validation (Fix 4) ── */
    do {
...
void display_marks_matrix(Student *list, int count)
{
    ...
    /* \u2500\u2500 Print column headers dynamically based on NUM_SUBJECTS (Fix 13) \u2500\u2500 */
    printf("  %-20s", "Name");
    ...
    /* \u2500\u2500 strtok() demonstration (Unit V): show subject name reference \u2500\u2500 */
    printf("\n  Subject Name Reference (strtok demo):\n");
    strcpy(labels_copy, SUBJECT_NAMES);
    token = strtok(labels_copy, ",");           /* strtok() usage (Unit V) */
    col_num = 1;
    while (token != NULL) {
        printf("  Subject-%d = %s\n", col_num++, token);
        token = strtok(NULL, ",");              /* subsequent strtok calls with NULL */
    }
}
...
void update_student(Student *list, int count)
{
    ...
    *temp_copy = *ptr;  /* copy current data into temp */
    ...
    /* Free the temporary copy (Unit V) */
    free(temp_copy);
    temp_copy = NULL;
...
int is_valid_input(Student *s, Student *list, int count, int self_roll)
{
    ...
    if (!valid) {
        session_fail_count++;
        printf(COLOR_YELLOW
            "  [Validation] Validation failures this session: %d\n"
...
void recursive_menu(Student **list, int *count, int *capacity)
{
    ...
    /* Fix 7: Recursion depth guard — prevent stack overflow */
    if (operations_counter >= 500) {
        printf(COLOR_YELLOW
            "  [Warning] Session limit reached (500 operations). "
    ...
    switch (choice) {
        ...
        case 5:     /* Search by Roll No — Binary Search */
            ...
            /* Sort by roll number first — required for binary search (Unit IV) */
            printf(COLOR_YELLOW
                "  [Warning] Binary search requires sorted roll numbers.\n"
                "  Array will be re-sorted by Roll No. "
                "Your CGPA sort (Option 6) will be lost.\n"
    ...
    /*
     * RECURSIVE CALL — calls itself for the next operation.
     * NOTE: In a production system, a while loop would be used
     * instead to avoid stack growth. This recursion is
     * intentional for demonstrating Unit III recursive functions.
     * The recursion depth is bounded by operations_counter check above.
     */
